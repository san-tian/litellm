...
def _without_header(
    ...
    """A copy of ``headers`` with ``name`` removed (case-insensitive), or None if nothing remains.
    Drops only that one header, keeping everything else forwarded.
    """
    if not headers:
        return None
...
def _resolve_openapi_tool_auth(
    ...
    if isinstance(per_server, dict):
        ...
        merged: Final = merge_mcp_headers(
            extra_headers=forwarded, static_headers=_without_header(per_server, DEFAULT_CREDENTIAL_HEADER)
        )
...
def _client_forwarded_authorization_headers(
    ...
    if extra_headers and _should_strip_caller_authorization(
        ...
        return _without_header(extra_headers, DEFAULT_CREDENTIAL_HEADER)
...
def _take_forwarded_authorization(
    ...
    return value, _without_header(headers, DEFAULT_CREDENTIAL_HEADER)
        ...
        *,
        ...
        match await provider.resolve_credentials(to_subject(user_api_key_auth, subject_token), spec):
            case Ok(auth):
                ...
                if not any(key.lower() == header_name.lower() for key in extra_headers):
                    return auth, extra_headers
                if isinstance(
                    spec.config,
                    ...
                    return auth, _without_header(extra_headers, header_name)
        ...
        if mcp_server.auth_type in (
            ...
            if mcp_server.has_client_credentials:
                ...
                if extra_headers and _should_strip_caller_authorization(
                    ...
                    extra_headers = _without_header(extra_headers, DEFAULT_CREDENTIAL_HEADER)
