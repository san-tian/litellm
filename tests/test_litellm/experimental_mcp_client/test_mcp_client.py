...
async def test_authorization_is_left_to_httpx_and_needs_no_guard():
    ...
    client = MCPClient(server_url="https://upstream.example.com/mcp", auth_type=MCPAuth.oauth2)
    assert client._guarded_credential_header(None) is None
    assert client._guarded_credential_header(StaticHeaderAuth("Bearer x")) is None
    assert client._guarded_credential_header(StaticHeaderAuth("Bearer x", header_name="esb-oauth")) == "esb-oauth"
