...
class TestMCPServerManager:
    ...
    def test_without_header_drops_only_the_named_header(self):
        from litellm.proxy._experimental.mcp_server.mcp_server_manager import _without_header
        from litellm.types.mcp import DEFAULT_CREDENTIAL_HEADER

        headers = {"Authorization": "Bearer a", "esb-oauth": "Bearer b", "X-Trace": "t"}
        assert _without_header(headers, "ESB-OAuth") == {"Authorization": "Bearer a", "X-Trace": "t"}
        assert _without_header(headers, DEFAULT_CREDENTIAL_HEADER) == {"esb-oauth": "Bearer b", "X-Trace": "t"}
    ...
    def test_without_header_drops_only_the_credential(self):
        # None / empty -> None
        assert _without_header(None, "Authorization") is None
        assert _without_header({}, "Authorization") is None
        # Only Authorization present -> nothing left -> None (case-insensitive)
        assert _without_header({"authorization": "Bearer x"}, "Authorization") is None
        # Authorization dropped, other headers kept
        assert _without_header({"Authorization": "Bearer x", "X-Trace-Id": "t"}, "Authorization") == {"X-Trace-Id": "t"}
